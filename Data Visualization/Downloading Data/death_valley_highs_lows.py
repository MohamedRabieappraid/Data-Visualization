...
with open(filename) as f:
    ...
    for row in reader:
        ...
        try:
            ...
            # we update the indexes to correspond to this file’s TMAX and TMIN positions.
            low = int(row[5])

        except ValueError:

                # If any data is missing, Python will raise a ValueError and we handle it by printing 
                # an error message that includes the date of the missing data.
                print(f"Missing data for {current_date}")

        # After printing the error, the loop will continue processing the next row. 
        # If all data for a date is retrieved without error, the else block  
...
# we update the title.
# Because we’re plotting information for a new location, we update the title to include the location on the plot, 
# and we use a smaller font size to accommodate the longer title.
